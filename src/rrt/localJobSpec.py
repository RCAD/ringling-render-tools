...
import rrt
from rrt.settings import JOBSPEC_DIR
from rrt.localFilesystem import get_share
...
class JobSpec(object):
    ...
    def _set_data(self, data):
        ...
        self._job_data['logs'] = os.path.join('D:\\hpc\\', 
                                                  getpass.getuser(), 
        ...
        self._job_data['stats'] = os.path.join('D:\\hpc\\', 
                                                  getpass.getuser(), 
                                                  '{job_id}', # we're going to let hpc-spool inject the job id into the path right before the job is submitted 
												  'stats',
    ...
    def submit_job(self, pause=False):
        ...
        cmd = '%s "%s"' % (r'python C:\Ringling\HPC\bin\local-spool-script.py', fp)
        if pause:
            cmd += " & pause"
        os.system(cmd)
